...
class Tailor_Made_Sync_Engine {
    ...
    public function sync_all() {
        ...
        $total_wp_posts = ( $wp_post_count->publish ?? 0 ) + ( $wp_post_count->draft ?? 0 );

        if ( empty( $synced_tt_ids ) && $total_wp_posts > 0 ) {
            // API returned 0 events but WP has posts — skip deletion to prevent mass-delete
