...
class Tailor_Made_API_Client {
    ...
    private function get( $endpoint, $params = array() ) {
        if ( empty( $this->api_key ) ) {
            return new WP_Error( 'no_api_key', __( 'Ticket Tailor API key not configured.', 'tailor-made' ) );
        }
    ...
    private function get_all( $endpoint, $params = array() ) {
        ...
        while ( true ) {
            $page++;
            if ( $page > $max_pages ) {
                return new WP_Error( 'pagination_limit', __( 'Pagination limit exceeded. Too many pages of results.', 'tailor-made' ) );
            }
    ...
    public function get_event( $event_id ) {
        return $this->get( '/events/' . urlencode( $event_id ) );
    }

    /** @return array|WP_Error */
    public function get_issued_tickets_for_event( string $event_id ) {
        return $this->get_all( '/issued_tickets', array(
    ...
    public function get_event_series_by_id( $series_id ) {
        return $this->get( '/event_series/' . urlencode( $series_id ) );
    }
